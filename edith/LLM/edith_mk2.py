...
#     try:
#         # Check if the model exists using 'ollama show'
#         result = subprocess.run(['ollama', 'show', model], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
#         if result.returncode != 0:
#             logging.warning(f"{model} model not found. Downloading...")
#             subprocess.run(['ollama', 'pull', model], check=True)
#             logging.info(f"{model} model downloaded successfully.")
#         else:
#             logging.info(f"{model} model is already present.")
#     except Exception as e:
#         logging.error("Failed to check or download model: %s", e)
#         sys.exit(1)
...
#     # Check if the model is present, if not, download it
#     check_and_download_model(model)

#     # Load the template if model is llama3.1
#     if model == 'llama3.1':
...
#         except Exception as e:
#             logging.error(f"Error handling document analysis: {e}")
...
#         try:
#             if response == "data_visualization_started":
#                 self.speak(res)
#                 logging.info("Data visualization started.")
#                 thread = threading.Thread(target=dvp)
#                 thread.daemon = True  # Set as a daemon thread
#                 thread.start()
#         except Exception as e:
#             logging.error(f"Error starting data visualization: {e}")

#         try:
#             if response == "task_manager_started":
#                 self.speak(res)
#                 thread = threading.Thread(target=task_manager)
#                 thread.daemon = True  # Set as a daemon thread
#                 thread.start()
#         except Exception as e:
#             logging.error(f"Error starting task manager: {e}")

#         try:
#             if response == "map_opened":
#                 self.speak(res)
#                 thread = threading.Thread(target=map_main)
#                 thread.daemon = True  # Set as a daemon thread
#                 thread.start()
#         except Exception as e:
...
#         try:
#             if response not in ['document_analysis_run', "log_request", "data_visualization_started", "task_manager_started", "map_opened"]:
