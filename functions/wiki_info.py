    ...
    def _fetch_info_from_wikipedia(self, subject, query_type, action=None):
        try:
            if query_type == 'summary':
                ...
                return f"{action} occurred in {match.group(1)}." if match else None
            elif query_type == 'birthplace':
                page = wikipedia.page(subject)
                content = page.content
                match = re.search(r'born.*?in\s+([\w\s,.]+)', content, re.IGNORECASE)
                return f"{subject} was born in {match.group(1)}." if match else None
            elif query_type == 'reason':
                page = wikipedia.page(subject)
                content = page.content
                match = re.search(rf'\b{action}\b(?:.*?)(because[\w\s,.]+)', content, re.IGNORECASE)
                return f"{action} {match.group(1)}." if match else None
                ...
                return f"{subject} is located at latitude {coordinates[0]} and longitude {coordinates[1]}." if coordinates else None
