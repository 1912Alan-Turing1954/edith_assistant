...
storage_status_phrases = [
    "You have {free_disk_space} of storage left.",
    "Your current storage usage is at {disk_usage}%.",
    "You’re using {disk_usage} of your {free_disk_space} storage capacity.",
    "You have {free_disk_space} free out of {free_disk_space} total.",
    "Storage usage is at {disk_usage} out of {free_disk_space} available.",
    "You’ve used {disk_usage} of your available storage space.",
    "Your remaining storage space is {free_disk_space}.",
    "Currently, you have {free_disk_space} free on your drive.",
    "You’re utilizing {disk_usage} out of {free_disk_space} available.",
    "You have {free_disk_space} left out of a total of {free_disk_space}."
]
...
def check_internet(host="8.8.8.8", port=53, timeout=2):
    ...
    try:
        ...
        return random.choice(witty_phrases["positive_network"])
    except socket.error:
        return random.choice(witty_phrases["network"])
...
def generate_storage_status_response(system_info):
    free_disk_space = system_info["Disk Info"][0]["Free Space"].replace("GB", "gigabytes")
...
def generate_cpu_usage_response(system_info):
    cpu_usage = max(system_info["CPU Info"]["CPU Usage"])
...
def generate_memory_usage_response(system_info):
    memory_usage = float(system_info["Memory Usage"][:-1])
...
def generate_disk_space_response(system_info):
    free_disk_space = system_info["Disk Info"][0]["Free Space"].replace("GB", "gigabytes")
    disk_usage = float(system_info["Disk Info"][0]["Disk Usage"][:-1])
